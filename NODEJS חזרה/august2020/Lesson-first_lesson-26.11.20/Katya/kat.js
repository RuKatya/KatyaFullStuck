...
const http = require('http'); //Imports http module from node
const fs = require('fs'); //file system
// const { count } = require('console');
let count = 1

// http.createServer((req, res) => { //creating server - request and respons בקשה ותגובה
...
http.createServer((req, res) => {
    ...
    console.log(`${count++}) ${req.url}`) // show with numbers what going after what

    if (req.url === '/') {
        fs.readFile('index.html', 'utf-8', (err, data) => {
            res.writeHead(200, { 'Content-Type': 'text/html' }) //https://www.restapitutorial.com/httpstatuscodes.html - codes of server
            res.write(data);
            res.end();
        })
